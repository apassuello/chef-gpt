...
@pytest.mark.parametrize("request_data,expected_error,expected_status", [
...
@responses.activate
def test_int_err_02_device_offline_error_to_503(
    client,
    auth_headers,
    ...
    assert "offline" in message_lower or \
           "connection" in message_lower or \
           "available" in message_lower, \
           f"Message should mention connectivity issue, got: {data['message']}"


@responses.activate
def test_int_err_03_device_busy_error_to_409(
    client,
    auth_headers,
    mock_anova_api_busy
):
    ...
    assert response.status_code == 409, \
        f"Expected 409 for busy device, got {response.status_code}"
...
def test_int_err_04_authentication_error_to_401(
    client,
    headers,
    description
):
