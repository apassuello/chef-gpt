...
def test_int_perf_01_health_check_fast(client):
    ...
    assert duration < 0.15, \
        f"Health check took {duration:.3f}s (threshold: 0.15s)"
...
def test_int_perf_02_start_cook_reasonable(
    ...
    assert duration < 2.5, \
        f"Start cook took {duration:.3f}s (threshold: 2.5s)"


@responses.activate
def test_int_perf_03_status_fast(
    client,
    auth_headers,
    mock_anova_api_success
):
    ...
    assert duration < 0.75, \
        f"Status query took {duration:.3f}s (threshold: 0.75s)"
